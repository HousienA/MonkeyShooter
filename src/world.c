...
Wall walls[23] = {
    {405, 635, 267, 745}, //wall 1
    {388, 404, 320, 740},  //wall 2
    {324, 387, 360, 699},
    {280, 323, 427, 630},  //left side circle wall ^
    {635, 663, 319, 693},
    {664, 700, 355, 677},
    {701, 719, 425, 655},
    {720, 750, 456, 545},    //mid circle wall
...
bool checkCollision(Character *character, Wall *walls, int num_walls) {
    ...
    for (int i = 0; i < num_walls; ++i) {
        if (character->dest.x + character->dest.w > walls[i].x_min && character->dest.x < walls[i].x_max &&
            (character->dest.y + character->dest.h / 2) > walls[i].y_min && character->dest.y < walls[i].y_max) {
