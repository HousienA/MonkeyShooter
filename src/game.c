...
int main(int argc, char *argv[])
{
    ...
    SDL_RenderClear(ctx.renderer);//clearar renderer
    SDL_Surface *menuImage = IMG_Load("resources/mMenu.jpeg");//laddar in menu jpg
    ctx.menu.tex = SDL_CreateTextureFromSurface(ctx.renderer, menuImage);
    SDL_FreeSurface(menuImage);
    SDL_RenderCopy(ctx.renderer, ctx.menu.tex, NULL, &ctx.background.dest);
        SDL_RenderPresent(ctx.renderer); //presenterar menu bilden tills vidare
    while(menu){
        state = SDL_GetKeyboardState(NULL); //scannar efter input
        if(state[SDL_SCANCODE_SPACE]>0){// om man trycker space hoppar den ur loopen, förstör bilden och går till running
            menu =FALSE;
            SDL_DestroyTexture(ctx.menu.tex);
        }
